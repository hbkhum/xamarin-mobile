    ...
    public static class ValidationHelper
    {
        ...
        private static void HideValidationFields(object model, Page page, string validationLabelSuffix = "Error")
        {
            ...
            foreach (var propertyName in properties)
            {
                ...
                {
                    var index = errorControlName.IndexOf('_');
                    errorControlName = $"{model.GetType().Name}" + "_" + errorControlName.Substring(index + 1); 
                    control = page.FindByName<Label>(errorControlName);
                    if (control != null)
                    {
                        control.IsVisible = false;
                    }
